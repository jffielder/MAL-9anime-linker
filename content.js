...
// Copyright (c) 2023 James Fielder
 ...
 * searchAndLinkMAL();
 */
async function MAL9LinkerSearchAndLinkMAL() {
  if (window.location.href.includes('9anime') && window.location.href.includes('/watch')) {
    ...
    MAL9LinkerSearchAnime(MAL9RemoveSpecialChars(anime_titles_array[0]))
      .then(results => {
        console.log(results)
 ...
 *
 * @example
 * MAL9LinkerSearchAnime('Cowboy Bebop').then(anime_list => {
 *   console.log(`Found ${anime_list.length} anime that match the search query.`);
 *   console.log(anime_list);
 * });
 */
async function MAL9LinkerMAL9LinkerSearchAnime(title_str) {
 ...
 * TODO: Use this function to link to MAL, preferably the search results page. 
 *
 ...
 * });
 */
async function MAL9LinkerTestAnimeURL(id) {
 ...
 * // Returns true
 */
function MAL9LinkerCompareTitles(result_title, page_title) {
 ...
 */
function MAL9LinkerAddLinkToTitle(mal_url, anime_title_html_element) {
  // link creation
 ...
 */
function MAL9LinkerMAL9RemoveSpecialChars(str) {
